...
(define (mult k l a b)
  ...
  (define (significand a)
    (if (equal? (cadr a) (make-list k 0)) (append (list 0) (caddr a)) (append (list 1) (caddr a))))
  (define (normalize a)
    (cond [(equal? (make-list k 0) (cadr a)) (if(= 0 (car (caddr a))) (list (car a) (cadr a) (slice (caddr a) 2 (+ l 1)))
                                                (list (car a) (u-add (length (cadr a)) (cadr a) (generate-1 (length (cadr a))))
                                                      (slice (caddr a) 2 (+ l 1))))]
          [(and (= 1 (car (caddr a))) (equal? (cadr a) (append (make-list (- l 1) 1) (list 0)))) (maxval k l)]
          [(= 1 (car (caddr a))) (list (car a) (u-add (length (cadr a)) (cadr a) (generate-1 (length (cadr a))))  (slice (caddr a) 2 (+ l 1)))]
          [(= 0 (car (caddr a))) (list (car a) (cadr a) (slice (caddr a) 3 (+ l 2)))]))
  ...
  (cond[(or (equal? a zero) (equal? b zero)) zero]
       ...
       [else (if (and (not (equal? (cadr a) (make-list k 0))) (not(equal? (cadr b) (make-list k 0))))
                      (normalize (list (if (or (and (= 0 (car a)) (= 0 (car b))) (and (= 1 (car a)) (= 1 (car b)))) 0 1)
                                       (cdr sum-exp) (u-mult (+ l 1) (significand a) (significand b))))
                      (let*[(temp (normalize1 (u-mult (+ l 1) (significand a) (significand b)) -1))]
                        (if (< (b2u (+ k 1) sum-exp) (cadr temp)) zero
                            (list (if (or (and (= 0 (car a)) (= 0 (car b))) (and (= 1 (car a)) (= 1 (car b)))) 0 1)
                                  (u-sub k (cdr sum-exp) (u2b k (cadr temp))) (car temp)))))]))
...
(define (divide k l a b)
  ...
  (define (normalize a)
    (cond [(equal? (make-list k 0) (cadr a)) (if(= 0 (car (caddr a))) (list (car a) (cadr a) (slice (caddr a) 2 (+ l 1)))
          ...
          [(and (= 1 (car (caddr a))) (equal? (cadr a) (append (make-list (- l 1) 1) (list 0)))) (maxval k l)]
          [(= 1 (car (caddr a))) (list (car a) (u-add (length (cadr a)) (cadr a) (generate-1 (length (cadr a))))  (slice (caddr a) 2 (+ l 1)))]
          [(= 0 (car (caddr a))) (list (car a) (cadr a) (cdaddr a))]))
  (define (normalize1 lst p)
    (cond [(null? (cadr lst)) (list lst p)]
          [(= 1 (cadr lst)) (list (slice lst 3 (+ l 2)) p)]
          [else (normalize1 (cdr lst) (+ p 1))]))
  (cond[(equal? b zero) (if (= 0 (car b)) (display "Plus Infinity") (display "Minus Infinity"))]
       ...
       [else (if (and (not (equal? (cadr a) (make-list k 0))) (not(equal? (cadr b) (make-list k 0))))
                      (normalize (list (if (or (and (= 0 (car a)) (= 0 (car b))) (and (= 1 (car a)) (= 1 (car b)))) 0 1)
                                       (cdr diff-exp) (car (u-div (+ l 1) (significand a) (significand b)))))
                      (let*[(temp (normalize1 (car (u-div (+ l 1) (significand a) (significand b))) -1))]
                        (if (< (b2u (+ k 1) diff-exp) (cadr temp)) zero
                            (list (if (or (and (= 0 (car a)) (= 0 (car b))) (and (= 1 (car a)) (= 1 (car b)))) 0 1)
                                  (u-sub k (cdr diff-exp) (u2b k (cadr temp))) (car temp)))))]))